...
import { deserializeTransferTx, makeQueryAllowanceTx,
    makeTransferStateTx, makeTransferToMany, makeTransferTx, makeWithdrawTsgTx, TST_CONTRACT
} from '../src/smartcontract/nativevm/tstAssetTxBuilder';
...
describe('test transfer asset', () => {
    ...
    test('test_transfer_asset_TST', async () => {
        const from = adminAddress;
        const to = new Address('AH9B261xeBXdKH4jPyafcHcLkS2EKETbUj');
        const tx = makeTransferTx('TST', from, to, 1, gasPrice, gasLimit);
    ...
    test('test_transfer_asset_TSG', async () => {
        const from = adminAddress;
        const to = new Address('AVDEiCVQzm7EffYH6vBQNXKYXR7RdVGNsA');
        const tx = makeTransferTx('TSG', from, to, 0.00003321 * 1e9, gasPrice, gasLimit);
        console.log(tx.serializeUnsignedData());
        signTransaction(tx, adminPrivateKey);
        const response = await socketClient.sendRawTransaction(tx.serialize(), false, true);
        // tslint:disable:no-console
    ...
    test('test transfer assets with sm2 account', async () => {
        ...
        const tx = makeTransferTx('TST', new Address('ATk57i8rMXFSBpHAdX3UQ4TNe48BBrfCoc'),
    ...
    test('test transfer with multi sign address', async () => {
        ...
        const tx = makeTransferTx('TST', mulAddr,
    ...
    test('test get allowance with tx', async () => {
        const from = adminAddress;
        const tx = makeQueryAllowanceTx('tsg', new Address(TST_CONTRACT), from);
    ...
    test('test get tsgs to withdraw with rest api', async () => {
        const addr = new Address('AMAqR8Y8JWngDk5QsoGpa7vPhu5Tu5KKeS');
        const result = await restClient.getAllowance('tsg', new Address(TST_CONTRACT), addr);
        console.log(result);
        expect(result).toBeTruthy();
    }, 10000);

    test('test withdraw tsg', async () => {
        const address = adminAddress;
        const amount = 1 * 1e9; // multiply 1e9 to set the precision
        const tx = makeWithdrawTsgTx(address, address, amount, address, gasPrice, gasLimit);
        signTransaction(tx, adminPrivateKey);
        const result = await restClient.sendRawTransaction(tx.serialize());
        console.log(result);
        expect(result).toBeTruthy();
    ...
    test('test amount number', () => {
        ...
        const tx = makeTransferTx('TSG', from, to, expectAmount, '500', '30000');
    ...
    test('transferToMulti', async () => {
        ...
        const tx = makeTransferToMany('TSG', adminAddress, [address1, address2], [0.01 * 1e9, 0.02 * 1e9], gasPrice, gasLimit);
        signTransaction(tx, adminPrivateKey);
        const response = await socketClient.sendRawTransaction(tx.serialize(), false, true);
        // tslint:disable:no-console
        console.log(JSON.stringify(response));
    ...
    test('transferStateTx', async () => {
        ...
        const payer = addrs[0];
        const tx = makeTransferStateTx('TSG', states, gasPrice, gasLimit, payer);
